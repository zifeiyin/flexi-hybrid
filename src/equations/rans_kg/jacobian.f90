...
fJac(6,1:7)= (/     -tke*v1,            tke,          0.,           0.,        0.,    v1,   0. /)
fJac(7,1:7)= (/     -omg*v1,            omg,          0.,           0.,        0.,    0.,   v1 /)
...
gJac(6,1:7)= (/     -tke*v2,           0.,           tke,          0.,       0.,    v2,   0. /)
gJac(7,1:7)= (/     -omg*v2,           0.,           omg,          0.,       0.,    0.,   v2 /)
...
gJac(6,1:7)= (/     -tke*v3,           0.,            0.,          tke,         0.,    v3,   0. /)
gJac(7,1:7)= (/     -omg*v3,           0.,            0.,          omg,         0.,    0.,   v3 /)
#else
absu=uu+vv
phi  = kappaM1*(0.5*absu + tke)
a1   = kappa * U(ENER)*sRho - phi
...
DO i=1,nDOF_loc
  ...
  IF ( muTOrig .gt. 10000. * muS ) THEN
    muTOrig = MIN( muTOrig, 10000. * muS )
    dmuTurb_drhok = 0.
    dmuTurb_drhog = 0.
    dDiffK_drhok  = 0.
    dDiffK_drhog  = 0.
    dDiffG_drhok  = 0.
    dDiffG_drhog  = 0.
  ELSE IF ((UPrim(TKE,i) .gt. 1.e-16 ) .and. (UPrim(OMG,i) .gt. 1.e-16)) THEN
    IF (muTOrig .gt. muSGS(1,i))  THEN 
      dmuTurb_drhok = muSGS(1,i) / ( UPrim(DENS,i) * UPrim(TKE,i) )
      dmuTurb_drhog = 0.
    ELSE 
      dmuTurb_drhok = Cmu * UPrim(OMG,i)**2
      dmuTurb_drhog = 2.0 * Cmu * UPrim(TKE,i) * UPrim(OMG,i)
    ENDIF 
    dDiffK_drhok  = invSigmaK * Cmu * UPrim(OMG,i)**2
    dDiffK_drhog  = 2.0 * Cmu * UPrim(TKE,i) * UPrim(OMG,i)
    dDiffG_drhok  = dDiffK_drhok
    dDiffG_drhog  = dDiffK_drhog
  ELSE 
    dmuTurb_drhok = 0.
    dmuTurb_drhog = 0.
    dDiffK_drhok  = 0.
...
#if PP_dim==3
    fJac(2,RHOK,i) = -tau(1,1) * dmuTurb_drhok
    fJac(3,RHOK,i) = -tau(1,2) * dmuTurb_drhok
    fJac(4,RHOK,i) = -tau(1,3) * dmuTurb_drhok
    fJac(2,RHOG,i) = -tau(1,1) * dmuTurb_drhog
    fJac(3,RHOG,i) = -tau(1,2) * dmuTurb_drhog
    fJac(4,RHOG,i) = -tau(1,3) * dmuTurb_drhog

    gJac(2,RHOK,i) = -tau(2,1) * dmuTurb_drhok
    gJac(3,RHOK,i) = -tau(2,2) * dmuTurb_drhok
    gJac(4,RHOK,i) = -tau(2,3) * dmuTurb_drhok
    gJac(2,RHOG,i) = -tau(2,1) * dmuTurb_drhog
    gJac(3,RHOG,i) = -tau(2,2) * dmuTurb_drhog
    gJac(4,RHOG,i) = -tau(2,3) * dmuTurb_drhog

    hJac(2,RHOK,i) = -tau(3,1) * dmuTurb_drhok
    hJac(3,RHOK,i) = -tau(3,2) * dmuTurb_drhok
    hJac(4,RHOK,i) = -tau(3,3) * dmuTurb_drhok
    hJac(2,RHOG,i) = -tau(3,1) * dmuTurb_drhog
    hJac(3,RHOG,i) = -tau(3,2) * dmuTurb_drhog
    hJac(4,RHOG,i) = -tau(3,3) * dmuTurb_drhog
#else
    fJac(2,RHOK,i) = -tau(1,1) * dmuTurb_drhok
    fJac(3,RHOK,i) = -tau(1,2) * dmuTurb_drhok
    fJac(4,RHOK,i) = 0.
  ...
  ! The energy equation depends on the KG working variable through both the effective viscosity and the effective thermal
  ! conductivity
  fJac(5,6,i) = -1.*dmuTurb_drhok * ((tau(1,1)*UPrim(2,i) + tau(1,2)*UPrim(3,i)) &
#if PP_dim==3
                                    + tau(1,3)*UPrim(4,i) &
#endif 
                                    + cp/PrSGS * gradUx(LIFT_TEMP,i))

  fJac(5,7,i) = -1.*dmuTurb_drhog * ((tau(1,1)*UPrim(2,i) + tau(1,2)*UPrim(3,i)) &
#if PP_dim==3
                                    + tau(1,3)*UPrim(4,i) &
#endif 
                                    + cp/PrSGS * gradUx(LIFT_TEMP,i))
  ...
  ! The energy equation depends on the KG working variable through both the effective viscosity and the effective thermal
  ! conductivity
  gJac(5,6,i) = -1.*dmuTurb_drhok * ((tau(2,1)*UPrim(2,i) + tau(2,2)*UPrim(3,i)) &
#if PP_dim==3
                                    + tau(2,3)*UPrim(4,i) &
#endif 
                                    + cp/PrSGS * gradUy(LIFT_TEMP,i))

  gJac(5,7,i) = -1.*dmuTurb_drhog * ((tau(2,1)*UPrim(2,i) + tau(2,2)*UPrim(3,i)) &
#if PP_dim==3
                                    + tau(1,3)*UPrim(4,i) &
#endif 
                                    + cp/PrSGS * gradUy(LIFT_TEMP,i))
  ...
  ! kg equation depends on k and g working variable itself
  fJac(6,6,i) = -dDiffK_drhok * gradUx(LIFT_TKE,i)
  fJac(6,7,i) = -dDiffK_drhog * gradUx(LIFT_TKE,i)
  fJac(7,6,i) = -dDiffK_drhok * gradUx(LIFT_OMG,i)
  fJac(7,7,i) = -dDiffK_drhog * gradUx(LIFT_OMG,i)

  gJac(6,6,i) = -dDiffK_drhok * gradUy(LIFT_TKE,i)
  gJac(6,7,i) = -dDiffK_drhog * gradUy(LIFT_TKE,i)
  gJac(7,6,i) = -dDiffK_drhok * gradUy(LIFT_OMG,i)
  gJac(7,7,i) = -dDiffK_drhog * gradUy(LIFT_OMG,i)
  
#if PP_dim==3
  hJac(6,6,i) = -dDiffK_drhok * gradUz(LIFT_TKE,i)
  hJac(6,7,i) = -dDiffK_drhog * gradUz(LIFT_TKE,i)
  hJac(7,6,i) = -dDiffK_drhok * gradUz(LIFT_OMG,i)
  hJac(7,7,i) = -dDiffK_drhog * gradUz(LIFT_OMG,i)
#endif
...
USE MOD_PreProc
USE MOD_Viscosity
USE MOD_Equation_Vars,ONLY:s43,s23,Cmu,invSigmaK,invSigmaG
...
DO i=1,nDOF_loc
  ...
  muTOrig= MIN( Cmu * UPrim(DENS,i) * MAX(UPrim(TKE,i),1.e-16) * MAX(UPrim(OMG,i),1.e-16)**2, 10000. * muS )
  diffK  = muS + invSigmaK * muTOrig
  diffG  = muS + invSigmaG * muTOrig
  ...
  fJacQx(5,1:8,i) = (/ 0., -muS*s43*v1,  -muS*v2,     -muS*v3,        0., -lambda,      0.,     0.    /)
  fJacQx(6,1:8,i) = (/ 0.,          0.,       0.,          0.,        0.,      0.,  -diffK,     0.    /)
  fJacQx(7,1:8,i) = (/ 0.,          0.,       0.,          0.,        0.,      0.,      0., -diffG    /)

  fJacQy(1,1:8,i) = 0.
  fJacQy(2,1:8,i) = (/ 0.,          0.,  muS*s23,          0.,        0.,      0.,      0.,     0.    /)
  fJacQy(3,1:8,i) = (/ 0.,        -muS,       0.,          0.,        0.,      0.,      0.,     0.    /)
  fJacQy(4,1:8,i) = 0.
  fJacQy(5,1:8,i) = (/ 0.,     -muS*v2,muS*s23*v1,         0.,        0.,      0.,      0.,     0.    /)
  fJacQy(6,1:8,i) = 0.
  fJacQy(7,1:8,i) = 0.
  ...
  fJacQz(5,1:8,i) = (/ 0.,     -muS*v3,       0.,  muS*s23*v1,        0.,      0.,      0.,     0.    /)
  fJacQz(6,1:8,i) = 0.
  fJacQz(7,1:8,i) = 0.
  ...
  gJacQx(4,1:8,i) = 0.
  gJacQx(5,1:8,i) = (/ 0.,  muS*s23*v2,  -muS*v1,          0.,        0.,      0.,      0.,     0.    /)
  gJacQx(6,1:8,i) = 0.
  gJacQx(7,1:8,i) = 0.
  ...
  gJacQy(5,1:8,i) = (/ 0.,     -muS*v1,-muS*s43*v2,   -muS*v3,        0., -lambda,      0.,     0.    /)
  gJacQy(6,1:8,i) = (/ 0.,          0.,       0.,          0.,        0.,      0.,  -diffK,     0.    /)
  gJacQy(7,1:8,i) = (/ 0.,          0.,       0.,          0.,        0.,      0.,      0., -diffG    /)

  gJacQz(1,1:8,i) = 0.
  gJacQz(2,1:8,i) = 0.
  gJacQz(3,1:8,i) = (/ 0.,          0.,       0.,     muS*s23,        0.,      0.,      0.,     0.    /)
  gJacQz(4,1:8,i) = (/ 0.,          0.,     -muS,          0.,        0.,      0.,      0.,     0.    /)
  gJacQz(5,1:8,i) = (/ 0.,          0.,  -muS*v3,  muS*s23*v2,        0.,      0.,      0.,     0.    /)
  gJacQz(6,1:8,i) = 0.
  gJacQz(7,1:8,i) = 0.
  ...
  hJacQx(5,1:8,i) = (/ 0.,  muS*s23*v3,       0.,     -muS*v1,        0.,      0.,      0.,     0.    /)
  hJacQx(6,1:8,i) = 0.
  hJacQx(7,1:8,i) = 0.
  ...
  hJacQy(5,1:8,i) = (/ 0.,          0.,muS*s23*v3,    -muS*v2,        0.,      0.,      0.,     0.    /)
  hJacQy(6,1:8,i) = 0.
  hJacQy(7,1:8,i) = 0.
  ...
  hJacQz(5,1:8,i) = (/ 0.,     -muS*v1,  -muS*v2, -muS*s43*v3,        0., -lambda,      0.,     0.    /)
  hJacQz(6,1:8,i) = (/ 0.,          0.,       0.,          0.,        0.,      0.,  -diffK,     0.    /)
  hJacQz(7,1:8,i) = (/ 0.,          0.,       0.,          0.,        0.,      0.,      0., -diffG    /)
  ...
  gJacQy(3,1:8,i) = (/ 0.,          0., -muS*s43,          0.,        0.,      0.,      0.,     0.    /)
  gJacQy(4,1:8,i) = 0.
  gJacQy(5,1:8,i) = (/ 0.,     -muS*v1,-muS*s43*v2,        0.,        0., -lambda,      0.,     0.    /)
  gJacQy(6,1:8,i) = (/ 0.,          0.,       0.,          0.,        0.,      0.,  -diffK,     0.    /)
  gJacQy(7,1:8,i) = (/ 0.,          0.,       0.,          0.,        0.,      0.,      0., -diffG    /)
...
!===================================================================================================================================
!> The Jacobian of the transformation from primitive to conservative variables without temperature
!===================================================================================================================================
...
dpdrho = KappaM1*(0.5*SUM(UE(EXT_VELV)*UE(EXT_VELV)) + UE(EXT_TKE))
#if PP_dim == 3
dedrho = (UE(EXT_VEL1)**2+UE(EXT_VEL2)**2+UE(EXT_VEL3)**2 + 2.0 * UE(EXT_TKE)) - dpdrho / KappaM1
#else
dedrho = (UE(EXT_VEL1)**2+UE(EXT_VEL2)**2 + UE(EXT_TKE) ) - dpdrho / KappaM1
#endif

Jac(1,1:7)= (/          1.,             0.,           0.,           0.,         0.,         0.,         0. /)
Jac(2,1:7)= (/UE(EXT_VEL1),   UE(EXT_DENS),           0.,           0.,         0.,         0.,         0. /)
Jac(3,1:7)= (/UE(EXT_VEL2),             0., UE(EXT_DENS),           0.,         0.,         0.,         0. /)
#if PP_dim==3
Jac(4,1:7)= (/UE(EXT_VEL3),             0.,           0., UE(EXT_DENS),         0.,         0.,         0. /)
Jac(5,1:7)= (/dedrho, UE(EXT_DENS)*UE(EXT_VEL1),UE(EXT_DENS)*UE(EXT_VEL2),UE(EXT_DENS)*UE(EXT_VEL3),1./KappaM1,UE(EXT_DENS),0./)
#else 
Jac(4,1:7)= 0.
Jac(5,1:7)= (/dedrho, UE(EXT_DENS)*UE(EXT_VEL1),UE(EXT_DENS)*UE(EXT_VEL2),0.,1./KappaM1,UE(EXT_DENS),   0. /)
#endif 
Jac(6,1:7)= (/UE(EXT_TKE),           0.,             0.,           0.,         0.,UE(EXT_DENS),        0.  /)
Jac(7,1:7)= (/UE(EXT_OMG),           0.,             0.,           0.,         0.,          0.,UE(EXT_DENS)/)
...
Jac(1,1:8)= (/          1.,             0.,           0.,           0.,         0.,         0.,         0.,         0. /)
Jac(2,1:8)= (/UE(EXT_VEL1),   UE(EXT_DENS),           0.,           0.,         0.,         0.,         0.,         0. /)
Jac(3,1:8)= (/UE(EXT_VEL2),             0., UE(EXT_DENS),           0.,         0.,         0.,         0.,         0. /)
#if PP_dim==3
Jac(4,1:8)= (/UE(EXT_VEL3),             0.,           0., UE(EXT_DENS),         0.,         0.,         0.,         0. /)
Jac(5,1:8)= (/dedrho, UE(EXT_DENS)*UE(EXT_VEL1),UE(EXT_DENS)*UE(EXT_VEL2),UE(EXT_DENS)*UE(EXT_VEL3),1./KappaM1,0.,UE(EXT_DENS),0./)
#else 
Jac(4,1:8)= 0.
Jac(5,1:8)= (/dedrho, UE(EXT_DENS)*UE(EXT_VEL1),UE(EXT_DENS)*UE(EXT_VEL2),0.,1./KappaM1,    0.,UE(EXT_DENS),        0. /)
#endif 
Jac(6,1:8)= (/UE(EXT_TKE),           0.,             0.,           0.,         0.,          0.,UE(EXT_DENS),        0. /)
Jac(7,1:8)= (/UE(EXT_OMG),           0.,             0.,           0.,         0.,          0.,          0.,UE(EXT_DENS)/)
...
#if PP_dim==3
dpdU = KappaM1*(0.5*SUM(UE(EXT_VELV)*UE(EXT_VELV))+UE(EXT_TKE))
#else 
dpdU = KappaM1*(0.5*(UE(EXT_VEL1)**2+UE(EXT_VEL2)**2)+UE(EXT_TKE))
#endif
...
#if PP_dim==3
Jac(4,1:7)= (/-UE(EXT_VEL2)*UE(EXT_SRHO),         0.,           0., UE(EXT_SRHO),         0.,          0.,          0.    /)
Jac(5,1:7)= (/              dpdU,-UE(EXT_VEL1)*KappaM1,-UE(EXT_VEL2)*KappaM1,-UE(EXT_VEL3)*KappaM1,KappaM1,-KappaM1,0.    /)
#else 
Jac(4,1:7)= 0.
Jac(5,1:7)= (/              dpdU,-UE(EXT_VEL1)*KappaM1,-UE(EXT_VEL2)*KappaM1, 0.,    KappaM1,         -KappaM1,     0.    /)
#endif

Jac(6,1:7) = (/-UE(EXT_TKE)*UE(EXT_SRHO),         0.,           0.,           0.,         0.,   UE(EXT_SRHO),       0.    /)
Jac(7,1:7) = (/-UE(EXT_OMG)*UE(EXT_SRHO),         0.,           0.,           0.,         0.,   0.,      UE(EXT_SRHO)     /)
...
#if PP_dim==3
dpdU(1) = KappaM1*(0.5*SUM(UE(EXT_VELV)*UE(EXT_VELV))+UE(EXT_TKE))
#else 
dpdU(1) = KappaM1*(0.5*(UE(EXT_VEL1)**2+UE(EXT_VEL2)**2)+UE(EXT_TKE))
#endif
...
#if PP_dim==3
Jac(4,1:7)= (/-UE(EXT_VEL2)*UE(EXT_SRHO),         0.,           0., UE(EXT_SRHO),         0.,          0.,          0.    /)
Jac(5,1:7)= (/           dpdU(1),-UE(EXT_VEL1)*KappaM1,-UE(EXT_VEL2)*KappaM1,-UE(EXT_VEL3)*KappaM1,KappaM1,-KappaM1,0.    /)
#else 
Jac(4,1:7)= 0.
Jac(5,1:7)= (/           dpdU(1),-UE(EXT_VEL1)*KappaM1,-UE(EXT_VEL2)*KappaM1, 0.,    KappaM1,         -KappaM1,     0.    /)
#endif
