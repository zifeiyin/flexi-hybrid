...
IF(MPIRoot.AND.doCalcTurbulence)THEN
  ...
  WRITE(UNIT_stdOut,formatStr)' Max turb   : ', kgnut(1:5:2)
  WRITE(formatStr,'(A,I2,A)')'(A14,',3,'ES18.9)'
  WRITE(UNIT_stdOut,formatStr)' Min turb   : ', kgnut(2:6:2)
  WRITE(formatStr,'(A,I2,A)')'(A14,',5,'ES18.9)'
  WRITE(UNIT_stdOut,formatStr)' Max source : ', kgnut(7:15:2)
  WRITE(formatStr,'(A,I2,A)')'(A14,',5,'ES18.9)'
  WRITE(UNIT_stdOut,formatStr)' Min source : ', kgnut(8:16:2)
END IF
...
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT/OUTPUT VARIABLES
REAL,INTENT(OUT)                :: kgnut(16)                   !> Conservative residuals
!----------------------------------------------------------------------------------------------------------------------------------
! LOCAL VARIABLES
INTEGER :: i
!==================================================================================================================================
...
kgnut(12) = -MINVAL(prodG(1,:,:,:,:))
kgnut(13) =  MAXVAL(dissG(1,:,:,:,:))
kgnut(14) = -MINVAL(dissG(1,:,:,:,:))
kgnut(15) =  MAXVAL(crossG(1,:,:,:,:))
kgnut(16) = -MINVAL(crossG(1,:,:,:,:))

#if USE_MPI
IF(MPIRoot)THEN
  CALL MPI_REDUCE(MPI_IN_PLACE,kgnut(1:16),16,MPI_DOUBLE_PRECISION,MPI_MAX,0,MPI_COMM_FLEXI,iError)
ELSE
  CALL MPI_REDUCE(kgnut(1:16) ,0          ,16,MPI_DOUBLE_PRECISION,MPI_MAX,0,MPI_COMM_FLEXI,iError)
END IF
#endif
