  ...
  CASE(3,4,9,91) ! Walls
    ...
    CASE(91)
      ...
      DO q=0,ZDIM(Nloc); DO p=0,Nloc
        ...
        gradUx_vNormal  = nv(1)*gradUx_master(LIFT_VEL1,p,q) &
                        + nv(2)*gradUx_master(LIFT_VEL2,p,q) &
                        + nv(3)*gradUx_master(LIFT_VEL3,p,q)

        gradUx_vTang1   = tv1(1)*gradUx_master(LIFT_VEL1,p,q)&
                        + tv1(2)*gradUx_master(LIFT_VEL2,p,q)&
                        + tv1(3)*gradUx_master(LIFT_VEL3,p,q)

        gradUx_vTang2   = tv2(1)*gradUx_master(LIFT_VEL1,p,q) &
                        + tv2(2)*gradUx_master(LIFT_VEL2,p,q) &
                        + tv2(3)*gradUx_master(LIFT_VEL3,p,q)

        gradUy_vNormal  = nv(1)*gradUy_master(LIFT_VEL1,p,q) &
                        + nv(2)*gradUy_master(LIFT_VEL2,p,q) &
                        + nv(3)*gradUy_master(LIFT_VEL3,p,q)

        gradUy_vTang1   = tv1(1)*gradUy_master(LIFT_VEL1,p,q) &
                        + tv1(2)*gradUy_master(LIFT_VEL2,p,q) &
                        + tv1(3)*gradUy_master(LIFT_VEL3,p,q)

        gradUy_vTang2   = tv2(1)*gradUy_master(LIFT_VEL1,p,q) &
                        + tv2(2)*gradUy_master(LIFT_VEL2,p,q) &
                        + tv2(3)*gradUy_master(LIFT_VEL3,p,q)

        gradUz_vNormal  = nv(1)*gradUz_master(LIFT_VEL1,p,q) &
                        + nv(2)*gradUz_master(LIFT_VEL2,p,q) &
                        + nv(3)*gradUz_master(LIFT_VEL3,p,q)

        gradUz_vTang1   = tv1(1)*gradUz_master(LIFT_VEL1,p,q) &
                        + tv1(2)*gradUz_master(LIFT_VEL2,p,q) &
                        + tv1(3)*gradUz_master(LIFT_VEL3,p,q)

        gradUz_vTang2   = tv2(1)*gradUz_master(LIFT_VEL1,p,q) &
                        + tv2(2)*gradUz_master(LIFT_VEL2,p,q) &
                        + tv2(3)*gradUz_master(LIFT_VEL3,p,q)

        ! Second: Transform to gradients w.r.t. wall-aligned directions, set boundary conditions
        gradUn_vNormal  = nv( 1)*gradUx_vNormal &
                        + nv( 2)*gradUy_vNormal+nv( 3)*gradUz_vNormal
        ...
        gradUt1_vTang1  = tv1( 1)*gradUx_vTang1 &
                        + tv1( 2)*gradUy_vTang1 &
                        + tv1( 3)*gradUz_vTang1
        gradUt1_vTang2  = tv1( 1)*gradUx_vTang2 &
                        + tv1( 2)*gradUy_vTang2 &
                        +tv1( 3)*gradUz_vTang2
...
ELSE
  ...
  CASE(9,91)
    ...
    DO q=0,PP_NZ; DO p=0,PP_N
      ...
      Flux(LIFT_TEMP:LIFT_OMG ,p,q) = UPrim_master(  6:8,p,q)
    END DO; END DO !p,q
  CASE(1) !Periodic already filled!
  CASE DEFAULT ! unknown BCType
    CALL Abort(__STAMP__,&
