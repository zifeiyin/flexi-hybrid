...
DO j=0,ZDIM(Nloc); DO i=0,Nloc
  ...
  U_LL(EXT_DTKE)=U_L(DTKE,i,j)
  U_LL(EXT_DOMG)=U_L(DOMG,i,j)
  ...
  U_RR(EXT_DTKE)=U_R(DTKE,i,j)
  U_RR(EXT_DOMG)=U_R(DOMG,i,j)
  ...
  Fout(DTKE,i,j)=F(DTKE)
  Fout(DOMG,i,j)=F(DOMG)
END DO; END DO
...
U_LL(EXT_DTKE)=U_L(DTKE)
U_LL(EXT_DOMG)=U_L(DOMG)
! rotate velocity in normal and tangential direction
...
U_RR(EXT_DTKE)=U_R(DTKE)
U_RR(EXT_DOMG)=U_R(DOMG)
! rotate momentum in normal and tangential direction
