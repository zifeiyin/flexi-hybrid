...
f(DTKE) = U(DTKE) * UPrim(VEL1)               ! rho*u*k
f(DOMG) = U(DOMG) * UPrim(VEL1)               ! rho*u*tg
...
g(DTKE) = U(DTKE) * UPrim(VEL2)               ! rho*v*k
g(DOMG) = U(DOMG) * UPrim(VEL2)               ! rho*v*tg
...
h(DTKE) = U(DTKE) * UPrim(VEL3)               ! rho*w*k
h(DOMG) = U(DOMG) * UPrim(VEL3)               ! rho*w*tg
#else
...
!==================================================================================================================================
!> Wrapper routine to compute the advection part of the RANS k-g fluxes for a single volume cell
!==================================================================================================================================
...
IMPLICIT NONE
!----------------------------------------------------------------------------------------------------------------------------------
! INPUT / OUTPkDiffEff = MAXUT VARIABLES
...
f(DTKE) = -kDiffEff*gradUx(LIFT_TKE)                       ! F_euler-(muS+sigmaK*muT)*k_x
f(DOMG) = -gDiffEff*gradUx(LIFT_OMG)                       ! F_euler-(muS+sigmaO*muT)*g_x
...
g(DTKE) = -kDiffEff*gradUy(LIFT_TKE)                       ! F_euler-(muS+sigmaK*muT)*k_y
g(DOMG) = -gDiffEff*gradUy(LIFT_OMG)                       ! F_euler-(muS+sigmaO*muT)*g_y
...
h(DTKE) = -kDiffEff*gradUz(LIFT_TKE)                       ! F_euler-(muS+sigmaK*muT)*k_z
h(DOMG) = -gDiffEff*gradUz(LIFT_OMG)                       ! F_euler-(muS+sigmaO*muT)*g_z
#else
! gradients of primitive variables are directly available gradU = (/ drho, dv1, dv2, dv3, dT /)
...
!==================================================================================================================================
!> Wrapper routine to compute the diffusive part of the RANS k-g fluxes for a single volume cell
!==================================================================================================================================
...
F(DTKE)=(U(DTKE)*UE(EXT_VEL1))               ! rho * u * tke
F(DOMG)=(U(DOMG)*UE(EXT_VEL1))               ! rho * u * g
...
F(DTKE)=(U(EXT_DTKE)*U(EXT_VEL1))            ! rho * u * tke
F(DOMG)=(U(EXT_DOMG)*U(EXT_VEL1))            ! rho * u * g
