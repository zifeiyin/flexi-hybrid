...
SELECT CASE(DepName_low)
  ...
  CASE("turbk")
    UCalc(:,iVarCalc) = UCalc(:,iRHOK) / UCalc(:,iDens)
  CASE("turbomega")
    ! UCalc(:,iVarCalc) = 1./(0.09*MAX(UCalc(:,iRHOG) / UCalc(:,iDens),1.e-16)**2)
    ! CALL FillField(UCalc(:,iVarCalc), omega(:,:,:,:), nVal)
    UCalc(:,iVarCalc) = UCalc(:,iRHOG) / UCalc(:,iDens)
  CASE("turbomega_real")
    CALL FillField(UCalc(:,iVarCalc), omega(:,:,:,:), nVal)
  CASE("pressure")
    DO i=1,PRODUCT(nVal)
      UE(EXT_SRHO) = 1./UCalc(i,iDens)
      UE(DENS)     =    UCalc(i,iDens)
      UE(EXT_MOM1) =    UCalc(i,iMom1)
      UE(EXT_MOM2) =    UCalc(i,iMom2)
      UE(EXT_MOM3) =    UCalc(i,iMom3)
      UE(EXT_ENER) =    UCalc(i,iEner)
      UE(EXT_RHOK) =    UCalc(i,iRHOK)
...
!    UCalc(:,iVarCalc) = UCalc(:,iPres)+0.5*UCalc(:,iDens)*UCalc(:,iVelM)**2
  ...
  CASE("fd")
    CALL FillField(UCalc(:,iVarCalc), fd(1,:,:,:,:), nVal)
