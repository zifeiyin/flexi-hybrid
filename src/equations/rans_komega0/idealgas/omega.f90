...
DO iElem=1,nElems
  DO k=0,PP_NZ; DO j=0,PP_N; DO i=0,PP_N
    ...
    IF (UPrim(OMG,i,j,k,iElem) .GE. 0.0) THEN
      ...
      xi = omega0Pos * ywall(1,i,j,k,iElem)**2 / nuS
      omega(i,j,k,iElem) = omega0Pos + (6.0 / Comega2) * nuS / ywall(1,i,j,k,iElem)**2 * EXP(-Cexp * xi)
    ELSE 
      omega0Pos = SQRT( &
      ...
      omega(i,j,k,iElem) = omega0Pos + (6.0 / Comega2) * nuS / ywall(1,i,j,k,iElem)**2 * EXP(-Cexp * xi)
    END IF
    IF (ywall(1,i,j,k,iElem).LE.1.0E-16) THEN
...
PPURE SUBROUTINE ComputeOmega2(UPrim, ywall, omega)
USE MOD_Globals
USE MOD_PreProc
USE MOD_Equation_Vars, ONLY: Comega2, Cexp
USE MOD_Viscosity
